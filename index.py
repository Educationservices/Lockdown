...
def auth_login():
    ...
    try:
        ...
        user = users_collection.find_one({'username': {'$regex': f'^{re.escape(clean_username)}from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
...
verified_signin_codes = {}  # {username: {'code': 'ABC123', 'verified_at': datetime, 'expires_at': datetime}}
...
def cleanup_expired_data():
    ...
    while True:
        try:
            ...
            # Clean up expired verified codes
            expired_verified_codes = [username for username, data in verified_signin_codes.items()
                                    if current_time > data['expires_at']]
            for username in expired_verified_codes:
                del verified_signin_codes[username]
            ...
            # Log cleanup stats
            if expired_verification or expired_signin_codes or expired_verified_codes:
                logger.info(f"Cleaned up {len(expired_verification)} expired verification codes, {len(expired_signin_codes)} expired signin codes, {len(expired_verified_codes)} expired verified codes")
...
# HTML Template with fixed quick signin input
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * {
        ...
        /* Special styling for code inputs */
        .code-input {
...
<body>
            ...
            <div id="verification-form" class="hidden">
                ...
                <form id="verificationForm">
                    <div class="form-group">
                        ...
                        <input type="text" id="verification-code" name="code" required maxlength="5" minlength="5" pattern="[0-9]{5}" placeholder="12345" class="code-input">
                    </div>
                    <button type="submit" class="btn">
            ...
            <div id="dashboard" class="hidden">
                ...
                <div class="user-info" style="margin: 20px 0;">
                    ...
                    <p style="margin-bottom: 15px; font-size: 0.9rem; color: #ccc;">
                        Enter a 6-character code received from an external source (like a game)
                    </p>
                    <div class="form-group">
                        <label for="external-code">Sign-in Code (6 characters)</label>
                        <input type="text" id="external-code" name="code" maxlength="6" minlength="6" pattern="^[A-Z0-9]{6}$" placeholder="ABC123" class="code-input" autocomplete="off" spellcheck="false" title="Enter 6-character code with letters and numbers">
                    </div>
                    <button class="btn btn-secondary" onclick="verifyExternalCode()" id="verify-code-btn">
                        Authenticate Code
                    </button>
                    <div id="code-status" class="hidden">
                        <div class="verification-code" id="verified-code-display">ABC123</div>
            ...
            <div id="quick-signin-form" class="hidden">
                ...
                <form id="quickSigninForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="quick-code" name="code" required maxlength="6" minlength="6" pattern="^[A-Z0-9]{6}$" placeholder="ABC123" class="code-input" autocomplete="off" spellcheck="false" title="Enter 6-character code with letters and numbers">
                    </div>
                    <button type="submit" class="btn">
    ...
    <script>
        ...
        // Enhanced input filtering for code inputs
        function setupCodeInputs() {
            // Quick sign-in code input
...
def auth_register():
    ...
    try:
        ...
        if users_collection.find_one({'username': {'$regex': f'^{re.escape(clean_username)}from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
from flask_cors import CORS
from werkzeug.security import generate_password_hash, check_password_hash
from pymongo import MongoClient
from datetime import datetime, timedelta
...
<body>
    ...
    <script>
        ...
        if users_collection.find_one({'email': {'$regex': f'^{re.escape(email)}from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
from flask_cors import CORS
from werkzeug.security import generate_password_hash, check_password_hash
from pymongo import MongoClient
from datetime import datetime, timedelta
...
def auth_verify():
    ...
    try:
        ...
        update_result = users_collection.update_one(
            {'username': {'$regex': f'^{re.escape(username)}from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
from flask_cors import CORS
from werkzeug.security import generate_password_hash, check_password_hash
from pymongo import MongoClient
from datetime import datetime, timedelta
...
def auth_resend():
    ...
    try:
        ...
        user = users_collection.find_one({'username': {'$regex': f'^{re.escape(username)}from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
from flask_cors import CORS
from werkzeug.security import generate_password_hash, check_password_hash
from pymongo import MongoClient
from datetime import datetime, timedelta
...
# NEW ENDPOINT: Check if a signin code was verified (for games to poll)
@app.route('/checkcodestatus', methods=['GET'])
@cooldown_required(1)
def check_code_status():
    """Check if a signin code has been verified by the user"""
    if not db_connected:
        return jsonify({'success': False, 'message': 'Service temporarily unavailable'}), 503
...
<body>
    ...
    <script>
        ...
        # Check if there's a verified code for this user
        if actual_username in verified_signin_codes:
            verified_data = verified_signin_codes[actual_username]
            
            # Check if still valid
            if datetime.utcnow() <= verified_data['expires_at']:
                return jsonify({
                    'success': True,
                    'verified': True,
                    'code': verified_data['code'],
                    'verified_at': verified_data['verified_at'].isoformat(),
                    'expires_at': verified_data['expires_at'].isoformat(),
                    'username': actual_username
                })
            else:
                # Clean up expired verified code
                del verified_signin_codes[actual_username]
        
        # Check if there's still a pending code
        if actual_username in signin_codes:
            ...
            if datetime.utcnow() <= signin_data['expires_at']:
                return jsonify({
                    ...
                    'message': 'Code generated but not yet verified by user'
                })
        ...
        return jsonify({
            ...
            'verified': False,
            'pending': False,
            'message': 'No active signin code'
        })
        
    except Exception as e:
        logger.error(f"Check code status error: {e}")
        return jsonify({'success': False, 'message': 'Failed to check code status'}), 500
...
def auth_verify_signin_code():
    ...
    try:
        ...
        # Move code to verified status (keep it for game to check)
        verified_signin_codes[current_username] = {
            'code': signin_data['code'],
            'verified_at': datetime.utcnow(),
            'expires_at': signin_data['expires_at']  # Keep same expiration
        }
        ...
        return jsonify({
            ...
            'code': verified_signin_codes[current_username]['code'],
            'expires_at': verified_signin_codes[current_username]['expires_at'].isoformat()
        })
    ...
    if db_connected:
        try:
            ...
            db_info = {
                ...
                'verified_signin_codes': len(verified_signin_codes),
...
if __name__ == '__main__':
    ...
    print("Starting Secure Lockdown Backend v2.1...")
    ...
    print("\nNew Features:")
    print("- Game polling system for code verification")
    print("- Fixed alphanumeric input handling")
    print("- Enhanced code input validation")
    print("- Separate verified codes storage")
    
    debug_mode = os.getenv('FLASK_ENV') == 'development'
    app.run(debug=debug_mode, host='0.0.0.0', port=port)from flask import Flask, jsonify, request, render_template_string, session, redirect, url_for
from flask_cors import CORS
from werkzeug.security import generate_password_hash, check_password_hash
from pymongo import MongoClient
from datetime import datetime, timedelta
